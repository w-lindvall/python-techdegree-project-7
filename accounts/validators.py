...
class SpecialCharacterValidator(object):
    ...
    def validate(self, password, user=None):
        ...
        if not any(character in special_characters for character in password):
            raise ValidationError(_('Your password must contain at least %(min_length)d special character.') % {
...
class NumberValidator(object):
    ...
    def validate(self, password, user=None):
        if not any(character.isdigit() for character in password):
            raise ValidationError(_('Your password must contain at least %(min_length)d digit.') % {
                                  'min_length': self.min_length})
...
class UpperLowerValidator(object):
    """Ensure password contains at least one uppercase and one lowercase letter."""
    def __init__(self, min_length=1):
        self.min_length = min_length
    ...
    def validate(self, password, user=None):
        if not any(character.isupper() for character in password) or \
            ...
            raise ValidationError(_('Your password must contain at least %(min_length)d uppercase and lowercase letter.') % {
                'min_length': self.min_length})
